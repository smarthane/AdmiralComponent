...
/**
 * @author smarthane
 * @time 2019/10/27 9:58
 * @describe 对 OkHttp 的请求和响应信息进行更规范和清晰的打印, 此类为框架默认实现, 以默认格式打印信息, 若觉得默认打印格式
 *  并不能满足自己的需求, 可自行扩展自己理想的打印格式
 */
public class DefaultFormatPrinter implements FormatPrinter {

    private static final String TAG = "AdmiralHttpLog";
    ...
    public void printJsonResponse(long chainMs, boolean isSuccessful, int code, @NonNull String headers, @Nullable MediaType contentType,
        ...
        bodyString = RequestInterceptor.isJson(contentType) ? StringUtils.jsonFormat(bodyString)
                : RequestInterceptor.isXml(contentType) ? StringUtils.xmlFormat(bodyString) : bodyString;
