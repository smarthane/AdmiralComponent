...
import com.smarthane.admiral.core.base.Platform;
...
public class AppManager {
    ...
    public void showSnackbar(String message, boolean isLong) {
        if (getCurrentActivity() == null && getTopActivity() == null) {
            LogUtils.warnInfo(TAG, "mCurrentActivity == null when showSnackbar(String,boolean)");
            return;
        }
        Completable.fromAction(new Action() {
            ...
            public void run() throws Exception {
                ...
                if (Platform.DEPENDENCY_SUPPORT_DESIGN) {
                    ...
                    AdmiralUtils.makeText(mApplication, message);
                }
            }
    ...
    public void startActivity(Intent intent) {
        if (getTopActivity() == null) {
            LogUtils.warnInfo(TAG, "mCurrentActivity == null when startActivity(Intent)");
    ...
    /**
     * 获取在前台的 {@link Activity} (保证获取到的 {@link Activity} 正处于可见状态, 即未调用 {Activity#onStop()}), 获取的 {@link Activity} 存续时间
    ...
    public Activity getTopActivity() {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when getTopActivity()");
            return null;
        }
    ...
    public void removeActivity(Activity activity) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when removeActivity(Activity)");
            return;
        }
        synchronized (AppManager.class) {
    ...
    public Activity removeActivity(int location) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when removeActivity(int)");
            return null;
        }
        synchronized (AppManager.class) {
    ...
    public void killActivity(Class<?> activityClass) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when killActivity(Class)");
            return;
        }
        synchronized (AppManager.class) {
    ...
    public boolean activityInstanceIsLive(Activity activity) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when activityInstanceIsLive(Activity)");
            return false;
        }
    ...
    public boolean activityClassIsLive(Class<?> activityClass) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when activityClassIsLive(Class)");
            return false;
        }
    ...
    public Activity findActivity(Class<?> activityClass) {
        if (mActivityList == null) {
            LogUtils.warnInfo(TAG, "mActivityList == null when findActivity(Class)");
            return null;
        }
        for (Activity activity : mActivityList) {
            if (activity.getClass().equals(activityClass)) {
