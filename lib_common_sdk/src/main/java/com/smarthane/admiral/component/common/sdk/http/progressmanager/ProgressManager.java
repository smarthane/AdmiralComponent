...
public final class ProgressManager {
    ...
    /**
     * 因为 {WeakHashMap} 将 {@code key} 作为弱键 (弱引用的键), 所以当 java 虚拟机 GC 时会将某个不在被引用的 {@code key} 回收并加入 {ReferenceQueue}
     * 在下一次操作 {WeakHashMap} 时,会比对 {ReferenceQueue} 中的 {@code key}
     * 将 {WeakHashMap} 中对应的 {@code key} 连同 {@code value} 一起移除,从而免去了手动 {Map#remove(Object)}
     * <p>
     * 建议你们在日常使用中,将这个 {@code key} 和对应的 {Activity}/{Fragment} 生命周期同步,也就使用全局变量持有
     ...
     * 既然 {@code key} 没被回收, 那 {WeakHashMap} 中的值也不会被移除
     */
    ...
    private static final String IDENTIFICATION_NUMBER = "?Admiral=";
    private static final String IDENTIFICATION_HEADER = "Admiral";
    ...
    /**
     * 设置 {ProgressListener#onProgress(ProgressInfo)} 每次被调用的间隔时间
     *
     ...
     * 会主动调用 {ProgressListener#onError(long, Exception)},但是有些错误并不是在它们内部发生的
    ...
    /**
     * 将 {okhttp3.OkHttpClient.Builder} 传入,配置一些本管理器需要的参数
     *
     * @param builder
     * @return
     */
     ...
     * 如已使用 {ProgressManager#with(OkHttpClient.Builder)},就不会用到此方法
     *
    ...
    /**
     * 如果 {@code url} 中含有 {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)}
     * 或 {#addDiffRequestListenerOnSameUrl(String, String, ProgressListener)} 加入的标识符,则将加入标识符
     ...
     * 请使用 {#addDiffResponseListenerOnSameUrl(String, ProgressListener)} 代替 {#addResponseListener}
     * {#addDiffResponseListenerOnSameUrl(String, ProgressListener)} 会返回一个加入了时间戳的新的 {@code url}
     * 请使用这个新的 {@code url} 去代替 {@code originUrl} 进行下载的请求即可 (当实际请求时依然使用 {@code originUrl} 进行网络请求)
     * <p>
     * {#addDiffResponseListenerOnSameUrl(String, ProgressListener)} 与 {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)}
     * 的区别在于:
     * {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)} 可以使用不同的 {@code key} 来自定义新的 {@code url}
     * {#addDiffResponseListenerOnSameUrl(String, ProgressListener)} 是直接使用时间戳来生成新的 {@code url}
     * <p>
     *
    ...
    /**
     * 当出现需要使用同一个 {@code url} 根据 Post 请求参数的不同而下载不同资源的情况
     * 请使用 {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)} 代替 {#addResponseListener}
     * 请使用 {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)} 会返回一个 {@code originUrl} 结合 {@code key} 生成的新的 {@code url}
     * 请使用这个新的 {@code url} 去代替 {@code originUrl} 进行下载的请求即可 (当实际请求时依然使用 {@code originUrl} 进行网络请求)
     * <p>
     * {#addDiffResponseListenerOnSameUrl(String, ProgressListener)} 与 {#addDiffResponseListenerOnSameUrl(String, String, ProgressListener)}
     * 的区别在于:
     ...
     * 请使用 {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 代替 {#addRequestListener}
     * {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 会返回一个加入了时间戳的新的 {@code url}
     * 请使用这个新的 {@code url} 去代替 {@code originUrl} 进行上传的请求即可 (当实际请求时依然使用 {@code originUrl} 进行网络请求)
     * <p>
     * {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 与 {#addDiffRequestListenerOnSameUrl(String, String, ProgressListener)}
     * 的区别在于:
     * {#addDiffRequestListenerOnSameUrl(String, String, ProgressListener)} 可以使用不同的 {@code key} 来自定义新的 {@code url}
     * {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 是直接使用时间戳来生成新的 {@code url}
     * <p>
     *
     * @param originUrl {@code originUrl} 作为基础并结合时间戳用于生成新的 {@code url} 作为标识符
     ...
     * 当出现需要使用同一个 {@code url} 根据 Post 请求参数的不同而上传不同资源的情况
     * 请使用 {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 代替 {#addRequestListener}
     * {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 会返回一个 {@code originUrl} 结合 {@code key} 生成的新的 {@code url}
     * 请使用这个新的 {@code url} 去代替 {@code originUrl} 进行上传的请求即可 (当实际请求时依然使用 {@code originUrl} 进行网络请求)
     * <p>
     * {#addDiffRequestListenerOnSameUrl(String, ProgressListener)} 与 {#addDiffRequestListenerOnSameUrl(String, String, ProgressListener)}
     * 的区别在于:
