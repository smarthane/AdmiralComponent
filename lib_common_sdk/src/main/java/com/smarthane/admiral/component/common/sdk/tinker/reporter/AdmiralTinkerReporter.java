...
/**
 * @author smarthane
 * @time 2019/11/10 19:17
 * @describe tinker data reporter
 */
public class AdmiralTinkerReporter {

    private static final String TAG = "Tinker.AdmiralTinkerReporter";
    ...
    public static void onLoaded(long cost) {
        ...
        if (cost < 0L) {
            LogUtils.errorInfo(TAG, "hp_report report load cost failed, invalid cost");
            return;
        }
        ...
        switch (errorCode) {
            case ShareConstants.ERROR_LOAD_EXCEPTION_DEX:
                if (throwable.getMessage().contains(ShareConstants.CHECK_DEX_INSTALL_FAIL)) {
                    ...
                    isCheckFail = true;
                    LogUtils.errorInfo(TAG, "tinker dex check fail:" + throwable.getMessage());
                } else {
                    reporter.onReport(KEY_LOADED_EXCEPTION_DEX);
                    LogUtils.errorInfo(TAG, "tinker dex reflect fail:" + throwable.getMessage());
                }
                break;
            case ShareConstants.ERROR_LOAD_EXCEPTION_RESOURCE:
                if (throwable.getMessage().contains(ShareConstants.CHECK_RES_INSTALL_FAIL)) {
                    ...
                    isCheckFail = true;
                    LogUtils.errorInfo(TAG, "tinker res check fail:" + throwable.getMessage());
                } else {
                    reporter.onReport(KEY_LOADED_EXCEPTION_RESOURCE);
                    LogUtils.errorInfo(TAG, "tinker res reflect fail:" + throwable.getMessage());
                }
                break;
    ...
    public static void onApplied(long cost, boolean success) {
        ...
        LogUtils.debugInfo(TAG, String.format("hp_report report apply cost = %d", cost));

        if (cost < 0L) {
            LogUtils.errorInfo(TAG, "hp_report report apply cost failed, invalid cost");
            return;
        }
    ...
    public static void onApplyPackageCheckFail(int errorCode) {
        ...
        LogUtils.debugInfo(TAG, String.format("hp_report package check failed, error = %d", errorCode));
